...
class Barrier(Sprite):
    img_list = [pg.image.load(f'images/Barrier/Barrier{n}.png') for n in range(8)]

    def __init__(self, game, ul, wh, img_list=None):
        self.game = game
        ...
        for row in range(wh[0]):
            for col in range(wh[1]):
                be = BarrierElement(game=game, img_list=img_list, ul=(ul[0] + col, ul[1] + row), wh=(4, 4))
